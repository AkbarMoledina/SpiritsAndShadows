...
public class Story {
    ...
    public static Area setUpWorld(Player player) {
        ...
        Event duelbookEvent = new ItemEvent(duelbook);

        Weapon spellbook = new Weapon("Spellbook", 1.2, 1.5);
        Event spellbookEvent = new ItemEvent(spellbook);

        Weapon battlestaff = new Weapon("Battlestaff", 1.8, 1.2);
        Event battlestaffEvent = new ItemEvent(battlestaff);

        Weapon spellstaff = new Weapon("Spellstaff", 1.2, 1.8);
        Event spellStaffEvent = new ItemEvent(spellstaff);
...
//        LearnSpellEvent learnFlameCoat = new LearnSpellEvent(flameCoat);
        ...
        Spell firestorm = new Spell("Firestorm", "Conjure a firestorm that encompasses the entire area, damaging yourself but also dealing damage over time to your enemy.", 15, 0, 4, 90, 40, Spell.spellType.PYRO, firestormEffects);
        ...
        Spell crystalCoffin = new Spell("Crystal coffin", "Buries your enemy in a large rectangle of ice", 0, 0, 6, 60, 50, Spell.spellType.ICE, crystalCoffinEffects);
        ...
        LearnSpellEvent learnLightningBurst = new LearnSpellEvent(lightningBurst);
        ...
        Spell gaiasWrath = new Spell("Gaia's wrath", "Call upon the wrath of Gaia to deal massive damage to the enemy.", 60, 0, 3, 90, 35, Spell.spellType.EARTH, null);
        LearnSpellEvent learnGaiasWrath = new LearnSpellEvent(gaiasWrath);

        List<SpellEffect> sanguinePactEffects = List.of(new SpellEffect(SpellEffect.spellEffect.SELF_SPELL_DMG, 1.6, 3, 100), new SpellEffect(SpellEffect.spellEffect.SELF_HP_CHANGE, -20, 0, 100));
        ...
        Spell runePrison = new Spell("Rune prison", "Damage and stun the enemy for 2 turn by trapping them in pure mana.", 10, 10, 4, 95, 50, Spell.spellType.SPIRIT, runePrisonEffects);
        LearnSpellEvent learnRunePrison = new LearnSpellEvent(runePrison);

        List<SpellEffect> ethrealMendEffects = List.of(new SpellEffect(SpellEffect.spellEffect.SELF_HP_CHANGE, 25, 3, 100), new SpellEffect(SpellEffect.spellEffect.SELF_INCOMING_DMG_MOD, 0.60, 3, 100));
        ...
        Spell siphonSoul = new Spell("Siphon soul", "Siphons the enemy's energy, healing you and causing them to deal reduced damage with their next attack.", 40, 0, 4, 100, 30, Spell.spellType.SHADOW, siphonSoulEffects);
        ...
        area01.setSearchEvent(duelbookEvent);
        ...
        area21.setSearchEvent(learnBloodthirst);
